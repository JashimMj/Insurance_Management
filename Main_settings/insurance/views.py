...
def previouslysende(request):
    ...
    mycursor.execute("""SELECT
              ...
              mrURL,
              umpStatus,
              depositStatus from ump_mr where RESPONSE is not null and mrDate between :fdate and :tdate and officeBranchCode=:bcode
              """, [fdate, tdate, a])
    myresults = mycursor.fetchall()
...
def MrserarchV(request):
    ...
    mycursor.execute("""SELECT
                  ...
                  mrURL,
                  umpStatus,
                  depositStatus from ump_mr where RESPONSE is not null and mrNumber=:mr
                  """, [mr])
    myresults = mycursor.fetchall()
...
def AcommentV(request,id=0):
    if id != 0:
        data=chatM.objects.filter(pk=id)
